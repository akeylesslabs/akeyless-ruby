  ...
  class UpdateMcpSecretBearerToken
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Akeyless::UpdateMcpSecretBearerToken` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Akeyless::UpdateMcpSecretBearerToken`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [accessibility, ara_enabled, bearer_token, enable_agentic_runtime_authority, enable_ai_quorum, input_rule, json, keep_prev_version, key, last_version, name, output_rule, token, uid_token, url].hash
    end
